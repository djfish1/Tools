        ...
        if doScatter:
            ...
            for icol, yCol in enumerate(range(1,data[fileName].shape[1])):
                line = ax.plot(data[fileName][:,0] / opts.xScale, data[fileName][:,yCol] / opts.yScale,
                        opts.marker, markersize=ms, markeredgecolor='None', markerfacecolor='auto', label=fileName + ':' + str(opts.yLabel[icol]))
        ylabel = opts.yLabel
        if len(ylabel) == 1:
            ylabel = ylabel[0]
        ax.set(xlabel=opts.xLabel, ylabel=ylabel, title=opts.title)
...
if __name__ == '__main__':
    ...
    elif len(opts.yCol) == 0:
        #mat.rcParams['axes.prop_cycle'] = mat.cycler('color',
        #        [[0.0, 0.0, 0.9], [0.0, 0.7, 0.0],
        #        [0.8, 0.0, 0.0], [0.8, 0.0, 0.8], [0.0, 0.8, 0.8]])
        # Viasat colors
        mat.rcParams['axes.prop_cycle'] = mat.cycler('color',
                (['009DE0', 'BED733', '003F73', '5EA215', '202E39', 'EF8201',
                  'A70066', '0000d0', 'd00000', '00b000', '9467bd', '8c564b',
    ...
    makePlots(data, opts)
    #print('Using backend:', mat.get_backend())
    plt.show()
